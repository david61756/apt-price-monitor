...
관심 단지의 신규 거래를 매일 감지해 **Discord 알림**을 보내고,
`state.json`에 누적 기록하며 **대시보드**(`docs/index.html`)를 자동 갱신합니다.

## 동작 방식

1. 매일 **08:00 / 14:00 / 18:00 KST** 로컬 Mac(launchd)이 `update.py`로 실거래+호가를 함께 갱신
   (GitHub Actions는 Mac이 꺼졌을 때를 대비한 실거래 클라우드 백업, 신고 지연 대비 **이번 달 + 지난달** 조회)
2. `config.yaml`의 관심 단지와 매칭 후 `state.json`/`quotes_state.json`과 비교해 **변동만** 감지
3. 변동 발견 시 Discord 알림 — 실거래 신규/해제, 호가 신규/인하/인상/내려감을 **한 건으로 요약**
4. 갱신된 `state.json`·`quotes_state.json`·대시보드를 자동 커밋

> **최초 실행** 시에는 기존 데이터 전체를 기준선으로 저장만 하고 알림은 보내지 않습니다
> (알림 폭탄 방지). 이후 실행부터 새로 등록된 변동만 알립니다.
...
python monitor.py              # 실거래만 실행
python monitor.py --dry-run    # 저장 없이 감지 결과만 확인
```
...
GITHUB_TOKEN=ghp_...           # 로컬 스크립트가 git push 인증에 사용
```
...
> Discord 알림이 필요 없으면 이 섹션은 건너뛰어도 됩니다.
> `DISCORD_WEBHOOK_URL`을 설정하지 않으면 알림 없이 기록·대시보드 갱신만 수행합니다.

1. Discord에서 알림을 받을 **채널** 선택 → **채널 설정(톱니) → 연동(Integrations) → 웹후크**
2. **새 웹후크** 생성 → 이름 지정 → **웹후크 URL 복사**
3. 복사한 URL을 `.env`의 `DISCORD_WEBHOOK_URL`에 넣습니다
   ```
   DISCORD_WEBHOOK_URL=https://discord.com/api/webhooks/<id>/<token>
   ```

> 알림은 로컬 Mac이 `update.py`를 실행할 때 전송되므로, `.env`(로컬)에만 넣으면 됩니다.
...
2. 저장소 **Settings → Secrets and variables → Actions → New repository secret**에서 등록:
   ...
   > GitHub Actions는 **실거래 데이터의 클라우드 백업**(Mac이 꺼졌을 때 대비)이라 Discord 알림은 보내지 않습니다.
   > 알림은 로컬 Mac의 `update.py`가 담당합니다.

3. **Actions 탭**에서 워크플로를 한 번 수동 실행(`Run workflow`)해 기준선을 만들어 두면 좋습니다
4. 이후 매일 **08:00 / 14:00 / 18:00 KST** 자동 실행됩니다
   - 시간 변경: [.github/workflows/monitor.yml](.github/workflows/monitor.yml)의 `cron` 수정 (UTC 기준, KST−9시간)
...
- 처음 실행은 기준선으로 저장만 합니다. 이후 실행부터 신규/인하/소멸을 감지·알림(Discord 선택)합니다.
...
```
update.py          # 정기 업데이트: 실거래+호가 갱신 후 변동 요약 Discord 전송 (launchd가 호출)
monitor.py         # 매매: 조회 → 신규 감지 → state/대시보드 갱신
