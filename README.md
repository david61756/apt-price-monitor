...
비공식 API라 로그인 세션이 필요합니다. **로그인된 브라우저에서 한 번만** 복사하면 됩니다:

1. 크롬에서 네이버 로그인 → `https://new.land.naver.com/complexes/2493` 접속
2. **F12(개발자도구) → Network 탭** → 페이지에서 매물 목록이 보이게 스크롤
3. 요청 목록에서 **`articles/complex/2493...`** 항목 클릭
4. **Headers**에서 `authorization: Bearer eyJ...` 값을 복사 → `.env`의 `NAVER_AUTH=` 에 붙여넣기
   (선택: `cookie:` 값 전체를 `NAVER_COOKIE=` 에)
   - 더 쉽게: 그 요청을 우클릭 → **Copy → Copy as cURL** 해서 저에게 주시면 제가 채워드립니다
5. 토큰은 시간이 지나면 만료됩니다 → 호가가 0건/인증오류로 나오면 위 과정으로 갱신
